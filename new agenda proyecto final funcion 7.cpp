...
void buscarPersona(DatosPersona persona[], int size) {
    string bus;
    cout << "Ingrese un nombre o codigo universitario: ";
    cin.ignore();
    getline(cin, bus);
    int enumer = 0;
    for (int i = 0; i < size; i++) {
        if (persona[i].escrito && (persona[i].nombre == bus || persona[i].codigo == bus)) {
            enumer++;
            cout << "\nResultado " << enumer << ": "
                 << "Nombre: " << persona[i].nombre << ", "
                 << "Genero: " << persona[i].genero << ", "
    ...
    if (enumer == 0) {
        cout << "No se encontro el resultado.\n";
    }
    cout << "Presione enter para continuar...";
    cin.get();
    limpiarPantalla();
